    ...
    export default {
        data() {
            return {
                ...
                periodTypeOptions: ['Day', 'Week', 'Month'],
                datePeriod: [],
                isPeriodSaved: false,
                filters: {
                    period: [],
        ...
        created() {
            this.datePeriod = this.filters.period = [moment().startOf('month').format('YYYY-MM-DD'), moment().endOf('month').format('YYYY-MM-DD')];
            this.filters.periodType = 'Day';

            this.loadUsers();
            this.loadPatients();

            this.usersSearchStream.pipe(
                debounceTime(500),
                distinctUntilChanged(),
            ).subscribe(query => this.loadUsers(query));
            this.patientsSearchStream.pipe(
                debounceTime(500),
                distinctUntilChanged(),
        ...
        methods: {
            loadUsers(query = null) {
                ...
                this.$store.dispatch("statistics/loadUsers", { params })
                    .finally(() => {
            ...
            loadPatients(query = null) {
                ...
                this.$store.dispatch("statistics/loadPatients", { params })
                    .finally(() => {
            ...
            getFilters() {
                ...
                return {
                    start_date: date[0],
                    end_date: date[1],
                    period: this.filters.periodType ? this.filters.periodType.toLowerCase() : 'day',
                    user_id: (this.filters.user && this.filters.user.id) ?  this.filters.user.id : null,
                    patient_id: (this.filters.patient && this.filters.patient.id) ?  this.filters.patient.id : null,
                };
            },
            savePeriod() {
